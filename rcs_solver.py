...
class PanelCoupledInfo:
    ...
    bc_kind: str
    # Surface impedance Z_s for Leontovich IBC, stored under the solver's
    # inward-pointing normal convention: the canonical IBC evaluated with
    # this Z_s is E_z = -Z_s * H_t (TM) and E_t = -Z_s * H_z (TE) along
    # the stored normal (see `_surface_robin_alpha`).  External material
    # tables given with the "physics" / outward-normal convention
    # (E_t = +Z_s * (n_out x H)) must be sign-flipped before being fed
    # here, or injected via the material library which does the sign
    # conversion on load.  Re(Z_s) >= 0 for passive lossy sheets.
...
class ImpedanceTaper:
    ...
    frequency-dependent fort.* table would require a 2-D (s, f) model and is
    ...
    _ALLOWED_KINDS = ("linear", "cosine", "exp")
...
class MaterialLibrary:
    ...
    def from_entries(
        ...
        for row in ibcs_entries:
            ...
            # Detect tapered-impedance format:
            #   flag  taper  <kind>  R_start  X_start  R_end  X_end
            # Legacy constant format:
            #   flag  R  X
            tokens = [str(t).strip() for t in row[1:] if str(t).strip() != ""]
            if tokens and tokens[0].lower() == "taper":
                if len(tokens) < 6:
                    raise ValueError(
                        f"IBC flag {flag}: tapered impedance requires "
                        "'taper <kind> R_start X_start R_end X_end' "
                        f"(got {len(tokens) - 1} data tokens after 'taper')."
                    )
                kind = tokens[1].strip().lower()
                r_start = _parse_float(tokens[2], 0.0)
                x_start = _parse_float(tokens[3], 0.0)
                r_end = _parse_float(tokens[4], 0.0)
                x_end = _parse_float(tokens[5], 0.0)
            ...
            z_real = _parse_float(tokens[0] if len(tokens) > 0 else 0.0, 0.0)
            z_imag = _parse_float(tokens[1] if len(tokens) > 1 else 0.0, 0.0)
    ...
    def get_impedance(self, flag: int, freq_ghz: float, arc_s: float | None = None) -> complex:
        if flag <= 0:
            return 0.0 + 0.0j
...
def validate_geometry_snapshot_for_solver(
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        pos_mat = _parse_flag(props[4])
        neg_mat = _parse_flag(props[5])
        ...
        if seg_type == 3:
            ...
            if pos_mat > 50:
                _resolve_fort_file(base_dir, pos_mat)
...
def _build_panels(
    ...
    for seg in segments:
        ...
        pos_mat = _parse_flag(props[4] if len(props) > 4 else 0)
        neg_mat = _parse_flag(props[5] if len(props) > 5 else 0)
        ...
        # internal convention (normal points into plus = pos_mat) for the
...
def _is_single_dielectric_body(infos: List[PanelCoupledInfo]) -> bool:
    ...
    solver does not evaluate.  A geometry containing TYPE 1 is rejected
    up front by ``_assert_no_type1_sheet`` so this predicate should never
    actually see a sheet, but the guard here is kept as belt-and-braces.
    """
    if _has_sheet(infos):
        return False
...
def _assert_no_type1_sheet(infos: List[PanelCoupledInfo]) -> None:
    """Raise a descriptive error if any element is a TYPE 1 sheet.

    TYPE 1 free-floating resistive/reactive sheet modelling is currently
    disabled because none of the available solver paths honour the sheet
    admittance q_plus_gamma correctly:

    - ``_solve_dielectric_indirect`` and ``_solve_multi_region_indirect``
      treat interfaces as pure-medium transmission and ignore q_plus_gamma
      entirely.  (Before this was guarded, TYPE 1 was routed here and
      silently returned a PEC-strip answer regardless of Z_s.)
    - The coupled trace formulation uses q_plus_gamma in its assembly, but
      its integral representation for the sheet case has pre-existing
      sign/normalization errors that produce unphysical results — Z_s does
      influence the answer but not in a way that matches the sheet physics.

    Rather than silently return wrong numbers, the solver fails fast when a
    TYPE 1 segment is encountered and points the user at the supported
    workaround (TYPE 2 with a tapered IBC).
    """
    if _has_sheet(infos):
        raise ValueError(
            "TYPE 1 free-floating resistive/reactive sheets are not currently "
            "supported by any validated solver path (the coupled trace "
            "formulation has pre-existing sign/normalization issues in the "
            "sheet case, and the specialized transmission solvers ignore "
            "sheet admittance).  For a tapered resistive treatment on a "
            "conducting body, use TYPE 2 segments with a tapered IBC row "
            "('flag taper <kind> R_start X_start R_end X_end') — that path "
            "is validated end-to-end, see test_taper_stairstep_cyl.py."
        )
...
def _geometric_sheet_endpoint_nodes(
    mesh: LinearMesh,
    infos: List[PanelCoupledInfo] | None = None,
) -> np.ndarray:
...
def _solve_te_sheet(
    ...
    At the sheet:  q = q_inc + N μ,  E_tangent = -(1/jωε) q,
    and K = Y · E_tangent gives:
        (N + jωε · Z_s · I) μ = -q_inc    →   (N + jk/η · Z_s · M) μ = -RHS_qinc

    (Using ωε = k/η with η = free-space impedance.)
...
def _solve_mixed_pec_dielectric(
    ...
    for nid in np.flatnonzero(~node_is_diel):
        ...
        if pol == 'TM' and is_pec:
            a_sys[row, :nnodes] = S_0[nid, :]
        elif pol == 'TE' and is_pec:
...
def _is_multi_region(infos):
    ...
    boundaries and would ignore the sheet's impedance.  Geometries
    containing sheets are rejected by ``_assert_no_type1_sheet`` before
    ...
    if not use_fmm:
        ...
        for mi, ifc in enumerate(ifaces):
            ...
            if ifc['pec_minus'] or ifc['pec_plus']:
                ...
                if np.any(tm_pec_mask):
                    block[tm_pec_mask, :] = S_sub[tm_pec_mask, :]
                Pdiag[dm[0]:dm[0]+nm, dm[0]:dm[0]+nm] = block
            else:
                d_sigma = dof_map[(mi, 'minus')]; d_tau = dof_map[(mi, 'plus')]
                k_m_val = region_props[r_m]['k']; k_p_val = region_props[r_p]['k']
                if pol == 'TM':
                ...
                Pdiag[d_sigma[0]:d_sigma[0]+nm, d_tau[0]:d_tau[0]+nm] = (
                    -inv_beta*(0.5*M_s + Kpp_sub))
                Pdiag[d_tau[0]:d_tau[0]+nm, d_sigma[0]:d_sigma[0]+nm] = Sm_sub
                Pdiag[d_tau[0]:d_tau[0]+nm, d_tau[0]:d_tau[0]+nm] = -Sp_sub
...
def solve_monostatic_rcs_2d(
    ...
    for freq_ghz in frequencies:
        ...
        # here (primarily TM PEC bodies, which the legacy coupled-trace
        # path mishandles).
        ...
        # --- Standard coupled formulation (TM PEC, or PEC+IBC without dielectrics) ---
...
def compute_surface_currents(
    ...
    use_te_robin = pol == 'TE' and _is_all_robin(coupled_infos)
    ...
    if use_multi:
        ...
        formulation = "TE Robin MFIE (SLP density)"
        ...
        # TM PEC fallback: use EFIE  S*sigma = -u_inc  to get the physical surface current.
        # The coupled trace formulation gives correct RCS but q=0 (wrong boundary current)
        # because its BC row enforces q=0 instead of u=0 for TM PEC.
        ...
        formulation = "EFIE (SLP density, TM PEC)"
